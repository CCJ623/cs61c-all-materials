...
void v_add_optimized_adjacent(double* x, double* y, double* z) {
  ...
  #pragma omp parallel
  {
    unsigned int num_thread = omp_get_num_threads();
    for (int i = omp_get_thread_num(); i < ARRAY_SIZE; i += num_thread)
      z[i] = x[i] + y[i];
  }
}
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
  ...
  #pragma omp parallel
  {
    unsigned int chunk_size = ARRAY_SIZE / omp_get_num_threads();
    int i = omp_get_thread_num() * chunk_size;
    int end;
